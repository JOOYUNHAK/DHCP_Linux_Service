...
char name_server[20]; char default_lease_time[10]; char max_lease_time[10]; int num;    unsigned char mac_addr[100];             
int CountIp();
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    unsigned char packet[BUFF_SIZE]; unsigned char buff_rcv[BUFF_SIZE];
    int capture_socket;  int offer_socket;    int on = 1;  
    struct iphdr *iphdr;    struct udphdr *udphdr;  struct bootp *bootp;
    struct ether_header *ethhdr;    struct sockaddr_in address;
   
    capture_socket = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ALL)); //캡쳐위한
    offer_socket = socket(PF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
    //setsockopt(raw_socket,IPPROTO_RAW, IP_HDRINCL, (char *)&on, sizeof(on));
   
    // 각 계층 헤더들
    ethhdr = (struct ether_header *)packet; 
    iphdr = (struct iphdr *)(packet +sizeof(struct ether_header) ); 
    udphdr = (struct udphdr *)(packet + sizeof(struct ethhdr)+sizeof(iphdr));
    bootp = (struct bootp *)(packet + sizeof(ethhdr)+sizeof(iphdr)+sizeof(struct udphdr));
    ...
    if(bind(server_socket, (struct sockaddr* )&server_addr, sizeof(server_addr))== -1) {
      ...
      exit(1);
       ...
       // while(1) {                                                                                                                                                                                
        str_len = recvfrom(capture_socket, buff_rcv , BUFF_SIZE, 0, (struct sockaddr *)&client_addr, &client_size);
        //DHCP_DISCOVER PACKET이 들어왔을경우
        if(buff_rcv[278] == 0x63 && buff_rcv[279] == 0x82 && buff_rcv[280] == 0x53 && buff_rcv[281] == 0x63 && buff_rcv[284] == 0x01)
        {
            strncpy(packet, &buff_rcv[6], 6);  //상대의 MAC주소를 복사해둔다.
            get_mac_addr(mac_addr);//나의 MAC주소 얻기위해
            printf("%02x %02x", mac_addr[0], mac_addr[1]);
        }
        printf("%02x", packet[0]);
        if(strcmp(inet_ntoa(client_addr.sin_addr), "0.0.0.0") == 0 && ntohs(client_addr.sin_port) == 0);
            ...
            strcpy(packet,buff_rcv);
           /* ethhdr->ether_dhost[0]=0x08;
            ethhdr->ether_dhost[0]=0x00;
            ethhdr->ether_dhost[0]=0x27;
            ethhdr->ether_dhost[0]=0x66;
            ethhdr->ether_dhost[0]=0x8c;
            ethhdr->ether_shost[0]=0x08;
            ethhdr->ether_shost[0]=0x00;
            ethhdr->ether_shost[0]=0x27;
            ethhdr->ether_shost[0]=0x79;
            ethhdr->ether_shost[0]=0x11;
            ethhdr->ether_shost[0]=0xf0;
            iphdr->version = 4;
            iphdr->ihl = 5;
            iphdr->tos = 0x10;
            iphdr->protocol = IPPROTO_UDP;
            iphdr->tot_len = sizeof(struct ether_header) + sizeof(struct iphdr) + sizeof(struct udphdr) + sizeof(struct bootp);
            iphdr->id = 0;
            iphdr->ttl = 60;
     ...
     // phdr->saddr = inet_addr("196.120.56.3");
          bootp->dhcp_lease.opt_value = htonl(600);
           ...
           //a sendto(raw_socket, packet, sizeof(struct udphdr)+sizeof(struct bootp), 0, (struct sockaddr *)&client_addr, sizeof(client_addr));
